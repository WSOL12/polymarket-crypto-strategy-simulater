...
function midFromSide(side: SideClobSnapshot): number | null {
  const bba = side.bestBidAsk;
  if (bba && Number.isFinite(bba.bestBid) && Number.isFinite(bba.bestAsk)) {
    return (bba.bestBid + bba.bestAsk) / 2;
  }
...
export function LivePriceChart({
  ...
  useEffect(() => {
    ...
    const uRaw = midFromSide(up);
    const dRaw = midFromSide(down);
  ...
  return (
    <div className="liveMidChart">
      ...
      {!streaming ? (
        ...
        <p className="muted small">Waiting for CLOB mid prices…</p>
      ) : (
        <div className="liveMidChartSvgWrap">
          <svg
            ...
            aria-label="Up and Down mid price over time"
          >
